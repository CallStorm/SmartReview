...
# 内容审核引用章节 / 知识库片段截断上限（当前章节正文截断见运行时设置 content_text_cap_chars）
...
def _content_node_worker(
    ...
    ldb = SessionLocal()
    try:
        content_text_cap = get_content_text_cap_chars(ldb)
        llm_max_output_tokens = get_llm_max_output_tokens(ldb)
        ...
        prompt = _content_prompt(
            ...
            global_rules,
            content_text_cap=content_text_cap,
        )
        llm_t0 = perf_counter()
        try:
            ...
            if step_id == "compilation_basis":
                ...
                def _basis_run(payload: tuple[dict[str, Any], str]) -> Any:
                    ...
                    try:
                        return _llm_review_execute(
                            ...
                            max_tokens=get_llm_max_output_tokens(ldb),
                        )
                    finally:
                        ldb.close()
                ...
                if not rp:
                    ...
                    sub, usage, log_lines, dbg = _llm_review_execute(
                        ...
                        max_tokens=get_llm_max_output_tokens(db),
                    )
                    for level, msg in log_lines:
                        _append_log(db, task, level, msg)
                    _merge_usage(token_usage_total, usage)
        ...
        if "image_review" in active and image_nodes:
            ...
            if not img_cfg.ready:
                ...
                def _img_node_worker(payload: tuple[dict, Any, Any, str, str, list[str]]) -> Any:
                    ...
                    ldb = SessionLocal()
                    try:
                        # 审核提示词落盘在 image_items.review_prompt_text（始终）；debug_prompts 仅调试开关
